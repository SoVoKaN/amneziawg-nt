...
SendJunkPackets(_In_ AWG_PEER *Peer)
{
    ...
    {
        /* Map each 16-bit draw into [Min, Max] with a multiply-shift rather than a modulo. The range is
         * ULONG because Min=0 with Max=MAXUSHORT makes it 65536, one past what a USHORT holds, and the
         * product then fills all 32 bits.
         */
