...
static_assert(
    ...
    "Peer->AllowedIPsCount struct mismatch");
static_assert((DWORD)AWG_IOCTL_PEER_HAS_PUBLIC_KEY == (DWORD)AMNEZIAWG_PEER_HAS_PUBLIC_KEY, "PEER_HAS_PUBLIC_KEY flag mismatch");
static_assert(
...
static_assert((DWORD)AWG_IOCTL_PEER_HAS_ENDPOINT == (DWORD)AMNEZIAWG_PEER_HAS_ENDPOINT, "PEER_HAS_ENDPOINT flag mismatch");
static_assert(
...
static_assert(
    ...
    "AllowedIp->Flags struct mismatch");
static_assert((DWORD)AWG_IOCTL_ALLOWED_IP_REMOVE == (DWORD)AMNEZIAWG_ALLOWED_IP_REMOVE, "ALLOWED_IP_REMOVE flag mismatch");
static_assert(sizeof(AWG_IOCTL_ADAPTER_STATE) == sizeof(AMNEZIAWG_ADAPTER_STATE), "Adapter state mismatch");
static_assert((DWORD)AWG_IOCTL_ADAPTER_STATE_DOWN == (DWORD)AMNEZIAWG_ADAPTER_STATE_DOWN, "Adapter state down mismatch");
...
AmneziaWGSetAdapterState(AMNEZIAWG_ADAPTER *Adapter, AMNEZIAWG_ADAPTER_STATE State)
{
    ...
    if (ControlFile == INVALID_HANDLE_VALUE)
        return FALSE;
    DWORD BytesReturned;
    if (!DeviceIoControl(ControlFile, AWG_IOCTL_SET_ADAPTER_STATE, &State, sizeof(State), NULL, 0, &BytesReturned, NULL))
    {
