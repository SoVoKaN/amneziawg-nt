...
CleanupOrphanedDevices(VOID)
{
    ...
    /* Step 2) ROOT\AMNEZIAWG devices that are not recognized by PNP, which were likely temporary nodes left around by driver installation. */
cleanupReg:
    HKEY EnumKey;
    if (RegOpenKeyExW(
            HKEY_LOCAL_MACHINE,
    ...
    for (DWORD Index = 0;;)
    {
        ...
        {
            ++Index;
            LOG_LAST_ERROR(L"Failed to remove phantom adapter %s", InstanceId);
        }
    }
    RegCloseKey(EnumKey);
