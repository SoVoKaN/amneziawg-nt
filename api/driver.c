    ...
    _Out_ AMNEZIAWG_ADAPTER_STATE *State)
{
    ...
    for (RequiredBytes = 512;; RequiredBytes = *ConfigurationBytes)
    {
        *Configuration = Alloc(RequiredBytes);
        ...
        if (DeviceIoControl(NdisHandle, AWG_IOCTL_GET, NULL, 0, *Configuration, RequiredBytes, ConfigurationBytes, NULL))
            break;
        Free(*Configuration);
        *Configuration = NULL;
...
DWORD WINAPI AmneziaWGGetRunningDriverVersion(VOID)
{
    return MaybeGetRunningDriverVersion(FALSE);
}

static BOOL EnsureAmneziaWGUnloaded(VOID)
{
    BOOL Loaded;
...
_Use_decl_annotations_
BOOL WINAPI AmneziaWGDeleteDriver(VOID)
{
    DWORD LastError = ERROR_SUCCESS;
